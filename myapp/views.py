...
def index(request):
    ...
    try:
        oferta_umumiy = CompanyDocument.objects.get(file_name="umumiy-offerta")
        oferta_snq = '#'
        oferta_fergana = CompanyDocument.objects.get(
            file_name="offerta-fargona")
            # oferta-qashq-sur
        oferta_qash_sur = CompanyDocument.objects.get(file_name="offerta-qashq-sur")
    ...
    form = ContactForm()
    context = {'form': form, "oferta_umumiy": oferta_umumiy,
               "oferta_snq": oferta_snq, "oferta_fergana": oferta_fergana, "oferta_qash_sur": oferta_qash_sur}
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if continue_proc:
                ...
                res = request_to_website(mxik_object_id)
                if res == False:
                    form = MxikForm()
...
def check_the_website() -> bool:
    try:
        ...
        if response.status_code == 200:
            return True
    except requests.exceptions.RequestException or requests.exceptions.ConnectionError or TimeoutError as e:
        return False
...
def request_to_website(file_id):
    try:
        ...
        count_row = count_rows(sheet_obj)
        count_column = sheet_obj.max_column

        # The number of rows with values is the number of times the loop ran
        ...
        for i in range(1, count_row + 1):
            ...
            if mxik_code:
                single_column_from_excel = get_column_from_excel(
                    sheet_obj, i, column_index_mxik + need_to_add)
        ...
        output_name = mxik_model.mxik_input.name.replace(
            '/mxik', '/output')
        deleted_list_name = mxik_model.mxik_input.name.replace(
            '/mxik', '/deleted')
        pd.DataFrame(result_list).to_excel(
            output_name, header=False, index=False)
        pd.DataFrame(deleted_list).to_excel(
            deleted_list_name, header=False, index=False)
        mxik_model.mxik_output = output_name
        mxik_model.mxik_deleted_list = deleted_list_name
        mxik_model.save()
        result_list.clear()
        deleted_list.clear()
        count = 0
...
def parse_data(data, column_excel, mxik_type):
    ...
    if data['data']:
        ...
        return True
    update_excel(column=column_excel, delete_column=True)
    return True


def update_excel(column, mxik_status=0, mxik_code=0, mxik_type=None, delete_column=False):
    # mxid_code does not exists
    if delete_column:
        deleted_list.append(column)
...
def get_column_from_excel(sheet, row_num, column_len):
    column = []
